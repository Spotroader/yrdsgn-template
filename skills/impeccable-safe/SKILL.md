...
description: Docs-only safe build of Impeccable — design, critique, audit, and polish frontend UI without bundled Node scripts or live browser servers. Use for websites, landing pages, e-commerce, dashboards, components, motion, typography, accessibility, and anti-AI-slop craft. Not for backend-only tasks. For live browser iteration, user must install full impeccable separately with explicit consent.
---

Designs and iterates production-grade frontend interfaces. Real working code, committed design choices, exceptional craft.

**Safe build:** no `scripts/` or `agents/`. Read [SAFE-MODE.md](SAFE-MODE.md) when a reference mentions Node scripts.

## Setup

You MUST do these steps before proceeding:

1. Read project context manually once per session: open `PRODUCT.md` and `DESIGN.md` if they exist (project root or `.impeccable/`). If `PRODUCT.md` is missing, stop and follow `reference/init.md` (manual setup only — skip live config and CSP scripts per SAFE-MODE.md).
2. If the user invoked a sub-command (`craft`, `shape`, `audit`, `polish`, ...), you MUST read `reference/<command>.md` next. Non-optional. When the reference mentions `node .../scripts/`, follow [SAFE-MODE.md](SAFE-MODE.md) instead.
...
5. **If the project is brand-new (no existing CSS tokens / theme / committed brand colors found in step 3)**, choose a brand seed from the brief and compose an OKLCH palette (bg, surface, ink, accent, muted) in CSS variables. **Skip only if step 3 found committed brand colors; identity-preservation wins.**
...
**Not in safe mode:** `live` (browser variant server) — see [SAFE-MODE.md](SAFE-MODE.md). Full impeccable required for live iteration.

### Routing rules

1. **No argument**: the user is asking "what should I do?" Make the menu context-aware. If `PRODUCT.md` is missing, finish init first. Otherwise inspect the project (open key HTML/CSS, git status, any `.impeccable/critique/*.md`), then lead with **2-3 highest-value next commands** with one-line reasons, followed by the full menu. **Never auto-run a command.**

   Heuristics (manual — no scripts):
   - Code exists but no `DESIGN.md` → `document`.
   - No prior critique snapshot → `critique <main page>`.
   - Stale or low-score critique → `polish` or re-run `critique`.
   - Dirty HTML/CSS in git → scoped `audit` or `polish` on those files.
   - Slop patterns visible in source (cream bg, gradient text, ghost cards) → matching refine command (`quieter`, `typeset`, `colorize`).
   - Do not recommend `live` in safe mode.

   Optional: if the user installed the npm CLI, they may run `npx impeccable detect` themselves; do not run it unless they ask.

   Keep it to 2-3 pointed picks. The menu stays the fallback.
...
`teach` is a deprecated alias for `init`: if the user types it, load [reference/init.md](reference/init.md) and proceed as if they ran `init` (safe mode: skip script steps in SAFE-MODE.md).